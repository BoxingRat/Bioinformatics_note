...
- 自动判断表达矩阵类型并选择差异分析方法：
  - raw count（整数矩阵）→ `DESeq2`
  - TPM/FPKM（连续小数）→ `limma`（基于 `log2(TPM+1)`）
...
### 依赖包
`GEOquery`, `DESeq2`, `limma`, `dplyr`, `tibble`, `stringr`, `ggplot2`, `ggrepel`, `clusterProfiler`, `org.Hs.eg.db`, `pheatmap`
