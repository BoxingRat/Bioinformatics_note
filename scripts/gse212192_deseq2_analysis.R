...
is_integer_matrix <- function(m) {
  all(abs(m - round(m)) < 1e-8, na.rm = TRUE)
}

run_differential <- function(expr_filt, col_data, control_label, treat_label, method = "auto") {
  if (method == "auto") {
    # TPM/FPKM 一般为小数；raw count 一般为整数且动态范围更大
    method <- if (is_integer_matrix(expr_filt) && max(expr_filt, na.rm = TRUE) > 50) "deseq2" else "limma_tpm"
  }

  if (method == "deseq2") {
    dds <- DESeqDataSetFromMatrix(countData = round(expr_filt), colData = col_data, design = ~ group)
    dds <- DESeq(dds)
    ...
    expr_norm <- assay(vst(dds, blind = FALSE))
    return(list(method = method, res_df = res_df, expr_norm = expr_norm))
  }

  # TPM/FPKM 等连续型表达值：用 limma 在 log2(TPM+1) 上做差异分析
  group_levels <- levels(col_data$group)
  group_mn <- make.names(group_levels)
  group_vec <- factor(make.names(as.character(col_data$group)), levels = group_mn)
  names(group_mn) <- group_levels

  design <- model.matrix(~0 + group_vec)
  colnames(design) <- group_mn
  contrast_str <- paste0(group_mn[[treat_label]], "-", group_mn[[control_label]])

  log_expr <- log2(expr_filt + 1)
  fit <- lmFit(log_expr, design)
  fit2 <- contrasts.fit(fit, makeContrasts(contrasts = contrast_str, levels = design))
  fit2 <- eBayes(fit2)
  ...
  res_df <- tt %>%
    ...
    transmute(
      gene_id = gene_id,
      baseMean = rowMeans(expr_filt[gene_id, , drop = FALSE]),
  ...
  list(method = method, res_df = res_df, expr_norm = log_expr)
}
...
message("[4/8] 差异分析（自动选择 DESeq2 或 limma-TPM）...")
col_data <- pheno %>%
  rownames_to_column("sample") %>%
  dplyr::select(sample, group) %>%
  column_to_rownames("sample")
diff_out <- run_differential(expr_filt, col_data, control_label, treat_label, method = "auto")
res_df <- diff_out$res_df
message(sprintf("差异分析方法: %s", diff_out$method))
write(sprintf("de_method=%s", diff_out$method),
      file = file.path(outdir, "run_config.txt"),
      append = TRUE)
